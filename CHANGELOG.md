...
- 新增低 I/O Gate：复用当前上下文和会话内已读集合；大于 200 行或 32 KiB 的文本先检查标题与大小，再按相关区段读取。
- 当 `active-task.md` 和目标源码已经承载所需事实时，不再默认创建重复的计划、交接或证据文档；原始日志只在审计或恢复需要时保留。
- 将始终加载的 `SKILL.md` 从 16.6 KB / 280 行压缩为不超过 14 KiB，并加入运行时合同门禁。
- 维护者本地同步完成后，默认只保留最近 3 份 `pmm-YYYYMMDD-HHMMSS` 安装备份；升级、迁移和其他运行目录不受影响。
- 修复多个任务对话同时启动 PMM 时的阻塞：另一个 active 主任务位于已检出的 worktree 时，当前未占用 worktree 的默认 `start` 自动路由为其 work-item；已有匹配分支 claim 时继续或恢复，不再重复创建 worktree。
- 同时发起的 `start` 使用五秒有界锁等待完成串行化；同一 worktree、paused/blocked 主任务、失配 claim 和未检出主分支仍失败关闭。
